...
class Environment(gym.Env):
    ...
    def step(self, action):
        ...
        # Calculate reward
        reward = self.worldModel.calcReward()

        # Determine if the episode is done (target is reached)
        done = self.worldModel.isResetted()
    ...
    def get_observation(self):
        ...
        if (len(stringContent) == 8):
            observation = self.worldModel.update(stringContent)
            self.oldObservation = {
            ...
            observation = {
                "world": self.oldObservation["world"], 
                "spatial": self.oldObservation["spatial"], 
                "inventory": self.oldObservation["inventory"],
                "reward": self.oldObservation["reward"]
            }
        return observation