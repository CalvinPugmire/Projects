...
class WorldModel:
    ...
    def calcReward(self):
        if self.reward[0] > 0.0 and self.oldReward[0] == 0.0:
            change = 0.0
            self.isReset = True
        else:
            change = (self.reward[0] - self.oldReward[0]) + (self.reward[1] - self.oldReward[1])

        if self.reward[0] == 0.0 and self.oldReward[0] > 0.0:
            change -= 10
        elif self.reward[0] > 0.0:
            change += 0.01
            pass
        
        self.oldReward = self.reward
    ...
    def hashShift(self, text):
        ...
        #min = hash_value % (2 * max + 1)
        #norm_value = (min - max) / max
        #return norm_value
