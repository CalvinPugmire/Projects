...
public class AgentEntity extends Mob implements InventoryCarrier {
    ...
    public AgentEntity(EntityType<? extends Mob> pEntityType, Level pLevel) {
        ...
        /*if (brainProcess == null && pLevel.isClientSide) {
            ProcessBuilder pb = new ProcessBuilder("python", "agent.py");
            pb.directory(new File("../agent/"));
            try {
                brainProcess = pb.start();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }*/
    ...
    public void tick () {
        ...
        if (newRewardInfo.get(0) != 0.0) {
            inputData = "START,,,"+newSightInfo+",,,"+newHearInfo+",,,"+newRewardInfo+",,,END";
        } else {
            inputData = "START,,,"+"[]"+",,,"+"[]"+",,,"+newRewardInfo+",,,END";
        }
