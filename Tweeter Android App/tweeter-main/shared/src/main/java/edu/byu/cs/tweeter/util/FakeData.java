...
public class FakeData {
     ...
     */
    private final AuthToken authToken = new AuthToken("super_secret_token");
        ...
        for (int i = 0; i < 2; ++i) {
            for (int j = 0; j < fakeUsers.size(); ++j) {
                ...
                String post = "Post " + i + " " + j +
                        ...
                        "\n" + url;
                try {
                    Status status = new Status(post, sender, getFormattedDateTime(), urls, mentions);
                    allStatuses.add(status);
                } catch (Exception ex) {
                    System.out.println("Failed to generate fake statuses because of exception: " + ex.getMessage());
                }
            }
        }
    }
    ...
    public String getFormattedDateTime() throws ParseException {
        ...
        return statusFormat.format(userFormat.parse(LocalDate.now() + " " + LocalTime.now().toString().substring(0, 8)));
    }
