...
Original file is located at
    https://colab.research.google.com/drive/1BAKuxTNT-LAEYcM79wpGMTIilpBRnu3T
...
def evect_approx1(x_0,k):
  m_0 = np.matrix([[1,1],[2,0]])
  m_k = np.matrix([[1,1],[2,0]])
  for i in range(k-1):
    m_k = np.matmul(m_k,m_0)
  return np.array(np.matmul(m_k,x_0))[0]
...
def norm_evect_approx1(x_0,k):
  m_0 = np.array([[1,1],[2,0]])
  x_k1 = x_0
  for i in range(k):
    w = m_0.dot(x_k1)
    x_k = x_k1
    x_k1 = w/np.linalg.norm(w)
  return x_k1,w[0]/x_k[0]
...
def ray_quotient(M,x_0,k):
  x_k,v_k = norm_approx_gen(M,x_0,k)
  x_k = M.dot(x_k).dot(x_k)/x_k.dot(x_k)
  return x_k
