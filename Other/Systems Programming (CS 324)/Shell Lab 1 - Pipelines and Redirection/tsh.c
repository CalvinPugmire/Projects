...
void eval(char *cmdline)
{
    ...
    int commandnum = parseargs(argv, cmds, stdin_redir, stdout_redir);

    builtin_cmd(argv);

    //begin execution of the commands in the pipeline
    ...
    for (int i=0; i<commandnum; i++) {
        if (i < commandnum-1 && commandnum > 1) {
            int piperesult = pipe(pipefd);
            if (piperesult < 0) {
                unix_error("Could not pipe()");
            }
        }
        ...
        if (pid == 0) { //child
            if (i == 0) {
                if (stdin_redir[0] != -1) {
                    FILE* input = fopen(argv[stdin_redir[0]], "r");
                    dup2(fileno(input), 0);
                    close(fileno(input));
                }
            } else {
