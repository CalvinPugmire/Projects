...
void eval(char *cmdline)
{
    ...
    //begin execution of the command
    ...
    if (pid == 0) {//child
        ...
        int executed = execve(argv[0], &argv[0], newenviron);
...
void do_bgfg(char **argv)
{
    if (strcmp(argv[0], "fg") != 0 && strcmp(argv[0], "bg") != 0) {
        printf("Command not passed to fg or bg");
        return;
    }
...
{
    struct job_t* jobref;
    while ((jobref = getjobpid(jobs, pid)) != NULL) {
        if (jobref->state == ST) {
            return;
        }
        sleep(1);
...
{
    int status;
    int sigpid = waitpid(-1, &status, WNOHANG | WUNTRACED);
    while (sigpid > 0) {
        if (WIFSTOPPED(status)) {
            printf("Job [%d] (%d) stopped by signal %d\n", pid2jid(sigpid), sigpid, WSTOPSIG(status));
            getjobpid(jobs, sigpid)->state = ST;
        } else if (WIFSIGNALED(status)) {
            printf("Job [%d] (%d) terminated by signal %d\n", pid2jid(sigpid), sigpid, WTERMSIG(status));
            deletejob(jobs, sigpid);
        } else if (WIFEXITED(status)) {
            deletejob(jobs, sigpid);
        }
        sigpid = waitpid(-1, &status, WNOHANG | WUNTRACED);
    }
