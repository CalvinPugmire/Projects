...
public class theRobot extends JFrame {
    ...
    void motionUpdate(int action) {
        ...
        for (int x = 0; x < mundo.width; x++) {
            for (int y = 0; y < mundo.height; y++) {
                ...
                double thisProb = probs[x][y]; //Probability we are in cell (x,y)

                // Distribute probability to 5 possible next cells (N,S,E,W,STAY)
                for( int a = 0; a < 5; a++) {
                    double actionProb = unintendedActionProb;  //An accidental action
                    if (a == action){
                        actionProb = intendedActionProb; //The intended action
                    }
                    ...
                    if(squareValid(newX,newY) && mundo.grid[newX][newY] != 1){
                        //Valid square, distribute prob to cell (newX,newY)
                        newProbs[newX][newY] += (actionProb * thisProb);
                    }
                    else{
                        //Invalid square, skip, add prob to this cell (x,y)
                        newProbs[x][y] += (actionProb * thisProb);
                    }
                }
            }
        }
    ...
    int maxAction(){
        ...
        for( int a=0;a<5;a++){
            double actionEV = getActionEV(max_x,max_y,a);
            if(actionEV > maxEV){
                maxEV = actionEV;
                bestAction = a;
            }
    ...
    int automaticAction() {
        ...
        //Try each action
        for( int a = 0; a<5; a++){
            //In each state see what the EV of that action will be, where EV is the probability that you are in that state * the EV of action in that state
            double actionEV = 0.0;
            for (int y = 0; y < mundo.height; y++){
                for (int x = 0; x < mundo.width; x++){
                    actionEV += (getActionEV(x,y,a)*probs[x][y]);
                }
            }
    ...
    void printValues(int i) {
        System.out.println("Values on iteration " + i + ":");
        for (int y = 0; y < mundo.height; y++){
            for (int x = 0; x < mundo.width; x++){
                switch(mundo.grid[x][y]){
                    case 0:
                        System.out.print(String.format("%.2f", Vs[x][y]));
                        break;
                    case 1:
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        System.out.print("G:" + String.format("%.1f", Vs[x][y]));
                        break;
                }
                System.out.print(" ");
            }
    ...
    double getActionEV(int x, int y, int a){
        //Move probabilities
        double intendedActionProb = moveProb;   
        double unintendedActionProb = (1.0 - moveProb) / 4.0;
        double actionEV = 0.0;
        ...
        if(a == STAY){
            stayProb = intendedActionProb;
        }

        //Now go over the other states that we could have ended up in and get their prob of transition * value
        for( int s = 0; s < 4; s++) {
            ...
            //Get the new cell for this action
            int newX = getNewX(s,x);
            int newY = getNewY(s,y);

            if(squareValid(newX,newY) && mundo.grid[newX][newY] != 1){
                //Valid square, this contributes into value of action
                actionEV += (actionProb * Vs[newX][newY]);
            }
            else{
        ...
        actionEV += (stayProb * Vs[x][y]);
        return actionEV;
    }

    void valueIteration() {
        //This will store the value function over the states
        Vs = new double[mundo.width][mundo.height];


        double epsilon = 0.0001;
        double goal_reward = 1.0;
        double stairwell_reward = -1.0;
        double empty_reward = -0.1;
        ...
        while(!converged){
            ...
            for (int y = 0; y < mundo.height; y++){
                for (int x = 0; x < mundo.width; x++){
                    switch(mundo.grid[x][y]){
                        case 0: //OPEN
                            //Do the value iteration update for this cell
                            double maxEV = -10000.0;

                            //Iterate over the actions we could choose
                            for(int a = 0; a < 5; a++){
                                //Get ev of this action in this square
                                double actionEV = getActionEV(x,y,a);
                                //Is this the maxEV action?
                                if(actionEV > maxEV){
                                    maxEV = actionEV;
                                }
                            }

                            newVs[x][y] = empty_reward + (discount_factor*maxEV);
                            break;
                        case 1: //WALL - skip this cell
                            continue;
                        case 2: //STAIRWELL
                            newVs[x][y] = stairwell_reward;
                            break;
                        case 3: //GOAL
                            newVs[x][y] = goal_reward;
                            break;
                    }

                    //Compute difference in this cell, use it to update max difference
                    double cur_difference = Math.abs(newVs[x][y] - Vs[x][y]);
            ...
            System.out.println("[VI - " + iterations + "]: D = " + max_difference);
