...
class MCTSNode:
    def __init__(self, board, player_number, parent):
        ...
        self.terminal = (len(self.moves) == 0) \
                        or is_winning_state(board, player_number) \
                        or is_winning_state(board, self.other_player_number)
        self.children = dict()
        for m in self.moves:
    ...
    def upper_bound(self, N):
        ...
        u_b = (self.w / self.n) + (self.c * np.sqrt(np.log(N) / self.n))
    ...
    def new_simulate(self):
        my_board = np.copy(self.board)
        turn = self.player_number
        while True:
            if is_winning_state(my_board, self.other_player_number):
                result = -1
                break
            if is_winning_state(my_board, self.player_number):
                result = 1
                break
            if len(get_valid_moves(my_board)) == 0:
                result = 0
                break
            act = random.choice(get_valid_moves(my_board))
            make_move(my_board, act, turn)
            if turn == self.player_number:
                turn = self.other_player_number
            else:
                turn = self.player_number
        self.w += result
        self.n += 1
    ...
    def simulate(self):
        ...
        #      1. Make a copy of the board to modify
            my_board = np.copy(self.board)
        #      2. Keep track of which player's turn it is (first turn is current node's self.player_number)
            turn = self.player_number
        ...
        #            3.3  If the game is over, store the result
                if is_winning_state(my_board, self.other_player_number):
                    result = 1
                elif is_winning_state(my_board, self.player_number):
                    result = -1
                elif len(get_valid_moves(my_board)) == 0:
                    result = 0
