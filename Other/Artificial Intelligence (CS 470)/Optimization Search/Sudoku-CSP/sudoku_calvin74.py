...
class Sudoku():
    ...
    def forward_check(self, row, column, value, mode='remove'):
        ...
        If [mode] == remove, then values should actually be removed from other cell domains.
...
def mrv(puzzle, unassigned):
    ...
    for i in unassigned:
        row = i[0]
        col = i[1]
        if len(puzzle.cells[row][col].domain) < min:
            min_un.clear()
            min_un.append((row, col))
            min = len(puzzle.cells[row][col].domain)
        elif len(puzzle.cells[row][col].domain) == min:
            min_un.append((row, col))
...
def order_values(puzzle, row, column):
    '''
    For the variable at the given row and column, return the values of this variable's domain,
    ...
    tuple_domain = []
    for i in domain:
        count = puzzle.forward_check(row, column, i, mode='count')
        tuple_domain.append((count, i))

    ord_tup_domain = sorted(tuple_domain, key=lambda x: x[0])

    ordered_domain = []
    for i in ord_tup_domain:
        ordered_domain.append(i[1])
...
def backtracking_search(puzzle):
    ...
    for i in ordered_values:
        ...
        test_puzzle.copy_puzzle(puzzle)

        # 4.2 Assign current value to selected variable (use assign_value())
        test_puzzle.cells[row][column].assign_value(i)
        ...
        valid = test_puzzle.forward_check(row, column, i, 'remove')
        ...
        solved_puzzle = backtracking_search(test_puzzle)
